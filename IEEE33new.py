...
class IEEE33_ENV:
    def __init__(self):
        ...
        self.dataset_index = 1  # 新增属性，用于跟踪当前使用的数据集

    def step(self, action, obse, tt) -> Tuple[np.ndarray, float, bool, float]:
        # action转换,action在-1~1之间，需要转换为实际值
        if self.dataset_index == 1:
            data_DER = sio.loadmat('data_24_WT_PV(1).mat')  # 第1个数据集
        elif self.dataset_index == 2:
            data_DER = sio.loadmat('data_24_WT_PV(2).mat')  # 第2个数据集
        elif self.dataset_index == 3:
            data_DER = sio.loadmat('data_24_WT_PV(3).mat')  # 第3个数据集
        ...
        # 获取数据集的行数
        num_rows = PVP_1.shape[0]  # 假设 PVP_1、PVP_2、PVP_3 的行数相同
        ...
        PVP[0] = max(0, PVP_1[random_row, tt])
        PVP[1] = max(0, PVP_2[random_row, tt])
        PVP[2] = max(0, PVP_3[random_row, tt])
        ...
        # 获取数据集的行数
        num_rows = WTP_1.shape[0]  # 假设 PVP_1、PVP_2、PVP_3 的行数相同
        ...
        WTP[0] = max(0, WTP_1[random_row, tt])
        WTP[1] = max(0, WTP_2[random_row, tt])
        WTP[2] = max(0, WTP_3[random_row, tt])
